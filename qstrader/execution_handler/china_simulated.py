...
class ChinaSimulatedExecutionHandler(AbstractExecutionHandler):
    """
    ...
    def calculate_china_commission(self, quantity, fill_price,action):
        """
        ...
        if(action == 'BOT'):
            ...
            commission = 0 # 0.0003* fill_price * quantity
        else: # action == "SLD"
            commission = 0 #0.0013 * fill_price * quantity
    ...
    def execute_order(self, event):
        ...
        if event.type == EventType.ORDER:
            ...
            exchange = "China"
            commission = self.calculate_china_commission(quantity, fill_price,event.action)
