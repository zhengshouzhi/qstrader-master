...
from .price_handler.tushare_data_csv_bar import TushareCsvBarPriceHandler
...
from .execution_handler.china_simulated import ChinaSimulatedExecutionHandler
...
class TradingSession(object):
    ...
    def __init__(
        ...
        title=None, benchmark=None,cycle = 'D'
    ):
        """
    ...
    def _config_session(self):
        ...
        elif self.price_handler =='tushare' and self.session_type =='backtest':

            self.price_handler = TushareCsvBarPriceHandler(
                self.config.CSV_DATA_DIR, self.events_queue,
                self.tickers, start_date=self.start_date,
                end_date=self.end_date,cycle=self.cycle
            )
        ...
        if self.execution_handler is None:
            self.execution_handler = ChinaSimulatedExecutionHandler(
                self.events_queue,
                self.price_handler,
                self.compliance
            )
    ...
    def start_trading(self, testing=False,filename =None):
        """
